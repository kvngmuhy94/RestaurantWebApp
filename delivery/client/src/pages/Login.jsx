...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // axios.post("http://localhost:8080/user/login", { email, password: pwd})
    // // .then(res => console.log(res.data.token))
    // .then(res => {
    //   const token = res.data.token
    //   sessionStorage.setItem('token', token)
    //   // let user = JSON.parse(sessionStorage.getItem('data', token));
    // })
        ...
        <div className="flex flex-col md:flex-row justify-evenly items-center h-half w-5/6 rounded-4xl gap-1 p-0 md:p-5 m-auto shadow-lg">
          ...
          <div className='w-full flex flex-col md:flex-row'>
            ...
            <p ref={errRef} className={errMsg ? "block text-red-500" : "hidden"} aria-live="assertive">{errMsg}</p>
              <h2 className='font-bold text-2xl text-primary pb-5 mx-0 md:mx-3 uppercase'>Login Form</h2>
