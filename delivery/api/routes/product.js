...
//get all product per company
router.get('/getall',auth.autheticateToken, (req,res,next)=>{
    var query="select p.id, p.name,p.description,p.price,p.image1,p.status,c.id as categoryId,c.name as categoryName from product as p INNER JOIN category as c where p.categoryId = c.id";
    connection.query(query,(err,results)=>{
        if(!err){
            return res.status(200).json(results);
        }
